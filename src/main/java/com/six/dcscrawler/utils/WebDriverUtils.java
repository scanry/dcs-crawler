...
public class WebDriverUtils {
	...
	public static List<WebElement> findElements(WebDriver driver, String selector) {
		...
		try {
			result = driver.findElements(By.cssSelector(selector));
		} catch (Exception e) {
	...
	public static void click(WebDriver webDriver, WebElement element) {
		int windowsHalfHeight=0;
		windowsHalfHeight=webDriver.manage().window().getSize().height/2;
		int y=element.getLocation().y-windowsHalfHeight;
		((JavascriptExecutor)webDriver).executeScript("scrollTo(0,"+y+")");  
		element.click();
	}
